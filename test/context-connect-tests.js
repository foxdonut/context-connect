...
var ctx = require("../src/context-connect");
...
describe("context-connect", function() {
  ...
  describe("not passing the expected arguments", function() {
    it("throws an error if there is no function", function() {
      expect(_.partial(ctx, "a", "b")).to.throw(Error);
    });

    it("throws an error if there are multiple functions", function() {
      expect(_.partial(ctx, "a", simpleFunction1, simpleFunction2)).to.throw(Error);
    });
